...
export async function PATCH(req: NextRequest, { params }: Params) {
  try {
    ...
    if (body.date_validite !== undefined) update.date_validite = body.date_validite || null;
    ...
    if (!data) return NextResponse.json({ error: "Pro forma introuvable" }, { status: 404 });
    return NextResponse.json({ facture_pro_forma: data });
  } catch (err) {
    console.error("[factures-pro-forma/PATCH]", err);
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  try {
    ...
    console.error("[factures-pro-forma/DELETE]", err);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}
