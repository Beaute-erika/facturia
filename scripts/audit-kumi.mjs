/**
 * Audit using kumi.systems Overpass mirror (overpass-api.de is currently overloaded).
 */
...
function findPhoneMatch(lead,osmEntries){if(lead._lat===undefined||lead._lon===undefined)return null;const leadLat=lead._lat,leadLon=lead._lon,leadNum=parseHousenumber(lead.adresse),leadStreet=parseStreet(lead.adresse);const candidates=[];for(const osm of osmEntries){const distM=haversineKm(leadLat,leadLon,osm.lat,osm.lon)*1000;if(distM>350)continue;if(distM<=300&&osm.housenumber&&osm.street&&leadNum&&osm.housenumber===leadNum){const streetSim=nameSimilarity(leadStreet,osm.street);if(streetSim>=0.35){candidates.push({phone:osm.phone,method:"address_exact",confidence:computeConfidence("address_exact",distM,streetSim),distM});continue;}}if(distM<=250&&osm.housenumber&&osm.postcode&&leadNum&&osm.housenumber===leadNum&&osm.postcode===lead.code_postal){candidates.push({phone:osm.phone,method:"address_cp",confidence:computeConfidence("address_cp",distM,0),distM});continue;}if(distM<=200&&osm.name){const sim=nameSimilarity(lead.nom,osm.name);let method=null;if(sim>=0.70&&distM<=150)method="name_high";else if(sim>=0.35&&distM<=150)method="name_medium";else if(sim>=0.20&&distM<=80)method="name_low";if(method)candidates.push({phone:osm.phone,method,confidence:computeConfidence(method,distM,sim),distM});}}if(!candidates.length)return null;candidates.sort((a,b)=>b.confidence!==a.confidence?b.confidence-a.confidence:a.distM-b.distM);const best=candidates[0];const secondary=candidates.find(c=>c.phone!==best.phone&&c.confidence>=40)?.phone??null;return{phone:best.phone,method:best.method,confidence:best.confidence,secondary};}
function enrichLeads(leads,osmEntries){if(!osmEntries.length)return;for(const lead of leads){const match=findPhoneMatch(lead,osmEntries);if(!match)continue;lead.telephone=match.phone;lead.phone_source="openstreetmap";lead.phone_confidence=match.confidence;lead.phone_match_method=match.method;lead.phone_secondary=match.secondary;}}
...
async function fetchOSMEntries(lat, lon, rayonKm) {
  ...
  const west = lon - delta / cosLat, east = lon + delta / cosLat;
  const query = `[out:json][timeout:25];(node["phone"](${south},${west},${north},${east});node["contact:phone"](${south},${west},${north},${east});way["phone"](${south},${west},${north},${east});way["contact:phone"](${south},${west},${north},${east}););out center;`;
  ...
  for (const mirror of OVERPASS_MIRRORS) {
    ...
    if (!resp.ok) { console.log(`   ⚠️ ${mirror} → HTTP ${resp.status}`); continue; }
    const data = await resp.json().catch(() => null);
    if (!data) continue;
    console.log(`   ✅ OSM via ${mirror.split("/")[2]}`);
...
async function runSearch(label, adresse, metier, rayon_km) {
  ...
  const leads = unique.map(e => {
    ...
    return { nom: e.nom_complet ?? "—", activite, adresse: e.siege.adresse ?? "",
      ...
      siret: e.siege.siret ?? null, siren: e.siren, distance_km: Math.round(distance_km*10)/10, score,
      source: "annuaire-entreprises", _lat: coords.lat, _lon: coords.lon };
  ...
  if (leads.length > 0 && withPhone.length === 0) {
    console.log(`\n⚠️  0 téléphone. 3 leads sans téléphone:`);
    leads.slice(0, 3).forEach((l, i) => console.log(`  ${i+1}. ${l.nom} — ${l.adresse} (${l.distance_km} km)`));
  }
}

console.log("🚀 AUDIT LEADS (kumi mirror) — " + new Date().toLocaleString("fr-FR"));
