/**
 * AUDIT LEADS — Production-equivalent test
 * Replicates search/route.ts logic exactly. No auth required.
 * Run: node scripts/audit-leads.mjs
 */

// ─── Utilities (mirrors route.ts) ────────────────────────────────────────────
...
function computeConfidence(method, distM, nameSim) {
  switch (method) {
    case "address_exact":
      return Math.max(80, Math.min(92, 92 - Math.round(distM / 10)));
    case "address_cp":
      return Math.max(75, Math.min(85, 85 - Math.round(distM / 12)));
    case "name_high":
      return Math.max(68, Math.min(85, Math.round((1 - distM / 200) * 40 + nameSim * 45)));
    case "name_medium":
      return Math.max(48, Math.min(68, Math.round((1 - distM / 200) * 30 + nameSim * 38)));
    case "name_low":
      return Math.max(32, Math.min(48, Math.round((1 - distM / 200) * 20 + nameSim * 28)));
  }
}
...
async function fetchOSMEntries(lat, lon, rayonKm) {
  ...
  if (!resp.ok) {
    console.log(`   ⚠️ Overpass HTTP ${resp.status}: ${await resp.text().catch(() => "")}`);
    return [];
  }
  const text = await resp.text();
  let data;
  try { data = JSON.parse(text); } catch {
    console.log(`   ⚠️ Overpass parse error: ${text.slice(0, 200)}`);
    return [];
  }
...
async function runSearch(adresse, metier, rayon_km) {
  console.log(`\n${"═".repeat(60)}`);
  console.log(`🔍 ${adresse} | ${metier} | ${rayon_km} km`);
  ...
  // 2. Extra departments for wide radius
  const deptSet = new Set([departement]);
  if (rayon_km > 30) {
    const R_EARTH = 111.32;
    const offsets = [
      [rayon_km / R_EARTH, 0], [-rayon_km / R_EARTH, 0],
  ...
  // 3. Parallel fetch: entreprises + OSM
  const MAX_PAGES = 3;
  const t0 = Date.now();
  ...
  const leads = unique
    .map((e) => {
      ...
      const score = Math.min(100, Math.max(0, Math.round(
        Math.max(0, 40 * (1 - distance_km / rayon_km)) + scoreRelevance(activite, metier)
      )));
      return {
        nom: e.nom_complet ?? "—",
        activite,
  ...
  if (leads.length > 0 && withPhone.length === 0) {
    console.log(`\n⚠️  Aucun téléphone trouvé. 5 leads sans téléphone:`);
    leads.slice(0, 5).forEach((l, i) => {
      console.log(`  ${i + 1}. ${l.nom} — ${l.adresse}, ${l.ville} (${l.distance_km} km)`);
    });
  }
}
...
const scenarios = [
  { adresse: "1 rue de Rivoli 75001 Paris", metier: "plombier", rayon_km: 10 },
  { adresse: "Place Bellecour 69002 Lyon", metier: "serrurier", rayon_km: 10 },
  { adresse: "Vieux-Port 13001 Marseille", metier: "boulanger", rayon_km: 10 },
];

console.log("🚀 AUDIT LEADS — " + new Date().toLocaleString("fr-FR"));

for (const s of scenarios) {
  await runSearch(s.adresse, s.metier, s.rayon_km);
  // Pause entre requêtes pour ne pas surcharger Overpass
